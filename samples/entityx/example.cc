...
// Nick Hildebrant - Minor modifications made from original 
// because all the balls were spawning at one point, causing
// them all to explode at once and then the app crashed
// I assume more entities spawned in 1 update cycle than the max
// allowed, but the cleanup on entity was never reached. whatever
...
float r(int a, float b = 0) {
    float x = static_cast<float>(std::rand() % (a * 1000) + b * 1000) / 1000.0f;
    return x;
}
...
public:
  ...
  void update(ex::EntityManager &es, ex::EventManager &events, ex::TimeDelta dt) override {
    ...
    for (int i = 0; i < count - c; i++) {
      ...
      // "Physical" attributes.
      entity.assign<Body>(
        sf::Vector2f((std::rand() % size.x), (std::rand() % size.y)),
        sf::Vector2f((std::rand() % size.x) / 10.f, (std::rand() % size.y) / 10.f));
